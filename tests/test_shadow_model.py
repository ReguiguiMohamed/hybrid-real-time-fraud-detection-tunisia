...
def test_registering_shadow_replaces_previous_model(tmp_path):
    ...
    statuses = {
        ...
        for r in session.query(ShadowModelRegistry).all()
    }
...
def test_label_join_analysis(tmp_path):
    ...
    manager.record_shadow_comparison(
        "TXN_1", 0.95, 0.92, champion_label=1, shadow_label=1, analyst_label="1"
    )
    manager.record_shadow_comparison(
        "TXN_2", 0.8, 0.3, champion_label=1, shadow_label=0, analyst_label="0"
    )
...
def test_latency_recording(tmp_path):
    ...
    manager.record_shadow_comparison(
        "TXN_1", 0.8, 0.75, latency_ms=12.5
    )
    manager.record_shadow_comparison(
        "TXN_2", 0.7, 0.68, latency_ms=8.3
    )
